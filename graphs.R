...
ggplot(data = p_votecounts %>% filter(state != "DC")) +
  geom_point(aes(x = p_state_decisive,
                 ...
                 color = p_vote_decisive), size = .8)+
  scale_color_gradient("Pr(your vote\nis decisive)",
                       low = "yellow", high = "red",
                       breaks = c(1e-9, 1e-7),
                       labels = c(expression(10^-9), expression(10^-7)),
                       trans = "log10") +
  geom_text_repel(aes(x = p_state_decisive,
                      y = p_state_tied_when_decisive,
                      label = state), size = 3) +
  scale_x_log10(name = "Chance that your state is needed for an electoral college win",
                limits = c(1e-3, 1),
                minor_breaks = NULL,
                breaks = c(.001, .01, .1, 1),
                labels = c(expression(10^-3), expression(10^-2), expression(10^-1), 1)) +
  scale_y_log10(name = "Chance that your state is tied, given\nthat its electoral votes are needed",
                limits = c(1e-10, 1e-4),
                minor_breaks = NULL,
...
ggplot() +
  ...
  scale_fill_gradient("Pr(individual vote\nis decisive)",
                      low = "yellow", high = "red",
                      breaks = c(1e-9, 1e-7),
                      labels = c(expression(10^-9), expression(10^-7)),
                      trans = "log10") +
...
p_votecounts %>%
  ...
  kable(., col.names = c("State", "Pr(State EV are needed)", "Pr(State vote is tied | State EV are needed)", "P(Individual vote is decisive)"))
